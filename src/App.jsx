...
import { 
  ...
  guardarInversoresExistentes,
  guardarModulosExistentes,
  guardarPropuestaInversores,
  guardarPropuestaModulos,
  guardarPropuestaAreas
...
// IMPORTANTE: Definidos FUERA de los componentes para evitar re-renders
// ============================================

// InputField Global - Para Paso 1 y general
const GlobalInputField = React.memo(({ label, campo, datos, onChange, tipo = 'text', placeholder = '', width = '100%', opcional = false, desdeSIPS = false, unidad = '', disabled = false }) => (
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      <input 
        ...
        style={{ 
          ...
          width, 
          padding: '10px 14px', 
          border: desdeSIPS && datos[campo] ? '2px solid #4CAF50' : '1px solid #DEE2E6', 
          borderRadius: '8px', 
          fontSize: '14px', 
          backgroundColor: disabled ? '#F5F5F5' : (desdeSIPS && datos[campo] ? '#F1F8E9' : 'white'),
          color: disabled ? COLOR_TEXT_LIGHT : COLOR_TEXT,
          outline: 'none', 
          boxSizing: 'border-box' 
        }}
      />
      {unidad && <span style={{ fontSize: '13px', color: COLOR_TEXT_LIGHT, minWidth: '50px' }}>{unidad}</span>}
    </div>
  </div>
));

// InputField con soporte SIPS - Para Paso 2
const GlobalInputFieldSIPS = React.memo(({ label, campo, campoSips, datos, onChange, tipo = 'text', placeholder = '', unidad = '', desdeSIPS = false }) => {
  ...
  return (
    <div style={{ marginBottom: '16px' }}>
      <label style={{ display: 'block', fontSize: '13px', fontWeight: '500', color: COLOR_TEXT, marginBottom: '6px' }}>
        {label}
        {tieneValorSips && <span style={{ marginLeft: '8px', fontSize: '10px', color: '#4CAF50', fontWeight: '400' }}>(SIPS)</span>}
      </label>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <input
          type={tipo}
...
// SelectField Global
const GlobalSelectField = React.memo(({ label, campo, campoSips, datos, onChange, opciones, opcional = false, desdeSIPS = false }) => {
  const valorSips = campoSips ? datos[campoSips] : null;
  const valor = datos[campo] !== undefined && datos[campo] !== '' ? datos[campo] : (valorSips ?? '');
  const tieneValorSips = valorSips !== undefined && valorSips !== null && valorSips !== '';
...
// ToggleField Global (Sí/No)
const GlobalToggleField = React.memo(({ label, campo, datos, onChange }) => (
  <div style={{ marginBottom: '16px' }}>
...
// SectionTitle Global
const GlobalSectionTitle = React.memo(({ children }) => (
...
const Paso1Proyecto = ({ datos, onChange }) => {
  ...
  // Usar componentes globales - estos son wrappers que pasan datos y handleChange
  const InputField = (props) => <GlobalInputField {...props} datos={datos} onChange={handleChange} />;
  const SelectField = (props) => <GlobalSelectField {...props} datos={datos} onChange={handleChange} />;
...
const Paso2Tarifa = ({ datos, onChange }) => {
  ...
  const InputField = (props) => <GlobalInputFieldSIPS {...props} datos={datos} onChange={handleChange} />;
  const SelectField = (props) => <GlobalSelectField {...props} datos={datos} onChange={handleChange} />;
  const SectionTitle = GlobalSectionTitle;
